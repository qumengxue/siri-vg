...
def train_one_epoch(
    ...
    for i, batch_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        #     model.module.transformer[0].encoder.eval()
        if args.masks:
            outputs = model(samples, captions)
        else:  
            memory_cache = model(targets, samples, captions, encode_and_save=True)
            outputs = model(targets, samples, captions, encode_and_save=False, memory_cache=memory_cache)
        ...
        # losses = sum(loss_dict[k] * (weight_dict[k]/2) for k in loss_dict.keys() if k in weight_dict)
