...
class Transformer(nn.Module):
    def __init__(
        ...
        self.sent_pos_attn = sent_pos_attn(d_model, dropout, 2)
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            # query_embed = self.sent_pos_attn(text_memory, query_embed)
...
class sent_pos_attn(nn.Module):
    def __init__(self, d_model=256, dropout=0.1, d_ffn=1024, activation="relu", n_heads=2):
        super().__init__()
    ...
    def forward(self, sent, query):
        # sent and queries attention
        q = query
        k = sent
        v = sent
        tgt = self.cross_attn(q, k, v)[0]
