...
from .gauss_map import make_all_points_gaussian, get_proposal_pos_embed, init_points
...
class MDETR(nn.Module):
    ...
    def __init__(
        ...
        if cascade_num > 0:
            self.class_embed = _get_clones(self.class_embed, cascade_num)
            self.bbox_embed = _get_clones(self.bbox_embed, cascade_num)
            self.transformer = _get_clones(self.transformer, cascade_num)
        ...
        self.add_feature = nn.Sequential(
            ...
            # nn.BatchNorm2d(1024),
            nn.Conv2d(1024, 2048, kernel_size=3, stride=2, padding=1, dilation=1),
            # nn.BatchNorm2d(2048),
        )

    def forward(self, targets, samples: NestedTensor, captions, encode_and_save=True, memory_cache=None):
        ...
        if encode_and_save:
            ...
            # layer2 + layer4 feature
            mid_src, _ = features[0].decompose()
            mid_src = self.add_feature(mid_src)
            src = src + mid_src

            query_embed = self.query_embed.weight
            memory_cache = self.transformer[0](
                self.input_proj(src),
                mask,
                query_embed,
                pos[-1],
            ...
            if self.cascade_num > 0:
                for i in range(self.cascade_num):
                    ...
                    if self.contrastive_align_loss:
                        ...
                        cas_proj_queries.append(proj_queries[-1])

                    outputs_class = self.class_embed[i](hs)
                    outputs_coord = self.bbox_embed[i](hs).sigmoid()
                    cas_outputs_class.append(outputs_class[-1])
                    cas_outputs_coord.append(outputs_coord[-1])
                    for j in range(outputs_class.shape[0]-1):
                        aux_outputs_class.append(outputs_class[j])
                        aux_outputs_coord.append(outputs_coord[j])
                        if self.contrastive_align_loss:
                            aux_proj_queries.append(proj_queries[j])

                    if i <= self.cascade_num:
                        final_coords, rand_points, query_embed = make_all_points_gaussian((100, 100), outputs_coord[-1], outputs_class[-1])
                        query_embed = self.pos_trans_norm(query_embed)
    
                out.update(
                    {
                        "pred_logits": outputs_class[-1],
                        "pred_boxes": outputs_coord[-1],
                        # "pred_logits": cas_outputs_class[0],
                        # "pred_boxes": cas_outputs_coord[0],
                    }
                )    
                ...
                if self.contrastive_align_loss:
                    ...
                    out["cas_outputs"] = [
                        ...
                        for a, b, c in zip(cas_outputs_class[:-1], cas_outputs_coord[:-1], cas_proj_queries[:-1])
                    ]
                else:
                    out["cas_outputs"] = [
                        {
                        ...
                        for a, b in zip(cas_outputs_class[:-1], cas_outputs_coord[:-1])
                    ]
                ...
                if self.aux_loss:
                    if self.contrastive_align_loss:
                        ...
                        out["aux_outputs"] = [
                            ...
                            for a, b, c in zip(aux_outputs_class[:], aux_outputs_coord[:], aux_proj_queries[:])
                        ]
                    else:
                        out["aux_outputs"] = [
                            {
                            ...
                            for a, b in zip(aux_outputs_class[:], aux_outputs_coord[:])
                        ]
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, positive_map, indices, num_boxes):
        ...
        # eos_coef = torch.full(loss_ce.shape, 0.5, device=target_sim.device)
        eos_coef[src_idx] = 1
        ...
        for i, ((idx_src, idx_tgt), tgt) in enumerate(zip(indices, targets)):
            ...
            for j, tok_list in enumerate(cur_tokens):
                for (beg, end) in tok_list:
                    ...
                    positive_map[i, idx_src[j], 0].fill_(True)
    ...
    def forward(self, outputs, targets, positive_map):
        ...
        if "cas_outputs" in outputs:
            for i, cas_outputs in enumerate(outputs["cas_outputs"]):
                indices = self.matcher(cas_outputs, targets, positive_map)
                for loss in self.losses:
                    kwargs = {}
                    l_dict = self.get_loss(loss, cas_outputs, targets, positive_map, indices, num_boxes, **kwargs)
                    l_dict = {k + f"_cas_{i}": v for k, v in l_dict.items()}
                    losses.update(l_dict)
        if "enc_outputs" in outputs:
            enc_outputs = outputs["enc_outputs"]
            indices = self.matcher(enc_outputs, targets, positive_map)
            for loss in self.losses:
                if loss == "masks":
                    # Intermediate masks losses are too costly to compute, we ignore them.
                    continue
                kwargs = {}
                l_dict = self.get_loss(loss, enc_outputs, targets, positive_map, indices, num_boxes, **kwargs)
...
def build(args):
    ...
    if args.aux_loss:
        aux_weight_dict = {}
        for i in range((args.dec_layers - 1)*args.cascade_num):
            aux_weight_dict.update({k + f"_aux_{i}": v for k, v in weight_dict.items()})
        weight_dict.update(aux_weight_dict)
    
    if args.cascade_num > 0:
        cascade_weight_dict = {}
        for i in range(args.cascade_num - 1):
            cascade_weight_dict.update({k + f"_cas_{i}": v for k, v in weight_dict.items()})
        weight_dict.update(cascade_weight_dict)
